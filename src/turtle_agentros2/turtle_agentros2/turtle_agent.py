...
class TurtleAgent(ROSA):
    ...
    def __init__(self, streaming: bool = False, verbose: bool = True):
        ...
        #streaming = self.get_parameter("streaming").get_parameter_value().bool_value
...
def main():
    ...
    turtle_agent = TurtleAgent(verbose=False)
    

    asyncio.run(turtle_agent.run())
    
    #rclpy.spin(turtle_agent)

    rclpy.shutdown()

if __name__ == "__main__":
    #rospy.init_node("rosa", log_level=rospy.INFO)
    rclpy.create_node('rosa')
    main()
